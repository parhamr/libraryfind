...
function statusCheck(url) {
	...
	if ((document.getElementById('stop_polling'))&&(document.getElementById('stop_polling')!=null)&&(document.getElementById('stop_polling').value!=null)&&(document.getElementById('stop_polling').value!='undefined')&&(document.getElementById('stop_polling').value=='false')) { 
	  new Ajax.Updater('searching_feedback', 
	    url.unescapeHTML(), 
		{asynchronous:true, evalScripts:true, parameters:'jobs='+ escape($('jobs').value)}); }
}
...
function spellCorrection(word) {
        getQueryElement().value=word;
         document.getElementById('search_form').submit();
		 return false;
}
function getQueryElement() {
	var query=document.getElementsByName('query[string]');
...
function showWait() {
  Element.show('waitimage');
  //ie7 hack
  setTimeout("document.getElementById('waitimage').innerHTML = document.getElementById('waitimage').innerHTML",200);
}
